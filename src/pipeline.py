...
def train_random_forest(X_train, y_train, tscv):
    ...
    rf_search = RandomizedSearchCV(rf, rf_params, n_iter=20, scoring='f1', cv=tscv, n_jobs=-1, random_state=42)
...
def train_xgboost(X_train, y_train, tscv):
    ...
    xgb_search = RandomizedSearchCV(xgb, xgb_params, n_iter=20, scoring='f1', cv=tscv, n_jobs=-1, random_state=42)
...
def train_lightgbm(X_train, y_train, tscv):
    ...
    lgbm_search = RandomizedSearchCV(lgbm, lgbm_params, n_iter=20, scoring='f1', cv=tscv, n_jobs=-1, random_state=42)
...
def shap_analysis(model, X_test, model_features, image_dir='images', sample_size=500):
    ...
    shap.summary_plot(shap_values, X_test[:sample_size], feature_names=model_features)